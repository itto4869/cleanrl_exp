...
RUN_GROUP="soap_rel_no_anneal_qbert_top5_seed_sweep"
export RUN_GROUP
...
if [[ -z "${JSON_PATH}" ]]; then
  JSON_PATH="experiments/soap_rel_no_anneal_top5.json"
fi
...
for config in "${CONFIGS[@]}"; do
  ...
  for seed in $SEEDS; do
    ...
    python experiments/ppo_atari_soap_rel_no_anneal.py \
      --exp-name "ppo_atari_soap_rel_no_anneal_hp_sweep_${tag}" \
      --seed "${seed}" \
      $args \
      "${COMMON_ARGS[@]}" \
      "$@"
