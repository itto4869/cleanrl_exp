...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run PPO Atari SOAP rollout grad clip from a config JSON."
    )
    parser.add_argument(
        "--config",
        default="experiments/grad_clip_config.json",
        help="Path to grad_clip_config.json with params and envs.",
    )
    parser.add_argument(
        "--total-timesteps",
        type=int,
...
def main():
    ...
    if not config["envs"]:
        raise ValueError("grad_clip_config.json envs is empty")

    script = "experiments/ppo_atari_soap_rollout_grad_clip.py"
    ...
    base_run_group = run_env.setdefault("RUN_GROUP", "grad_clip_config_sweep")
    run_label = "soap-rollout-grad-clip"
