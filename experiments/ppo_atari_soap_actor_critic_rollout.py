...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        x_scaled = x / 255.0
        actor_hidden = self.actor_network(x_scaled)
        logits = self.actor(actor_hidden)
        probs = Categorical(logits=logits)
        if action is None:
            action = probs.sample()
        critic_hidden = self.critic_network(x_scaled)
        return action, probs.log_prob(action), probs.entropy(), self.critic(critic_hidden)
...
if __name__ == "__main__":
    ...
    actor_params = list(agent.actor_network.parameters()) + list(agent.actor.parameters())
    critic_params = list(agent.critic_network.parameters()) + list(agent.critic.parameters())
    actor_optimizer = SOAP(
        params=actor_params,
        lr=args.learning_rate,
        normalize_grads=args.soap_normalize_grads,
        precondition_frequency=args.soap_precondition_frequency,
    )
    critic_optimizer = SOAPRolloutResetStats(
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Update critic preconditioner from the full rollout batch.
        critic_optimizer.zero_grad()
        full_value = agent.get_value(b_obs).view(-1)
        if args.clip_vloss:
        ...
        full_v_loss = full_v_loss * args.vf_coef
        full_v_loss.backward()
        critic_optimizer.update_preconditioner_from_grads()
        critic_optimizer.zero_grad()
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                for p in actor_params:
                    ...
                    actor_norm_sq += param_norm.item() ** 2
                actor_grad_norm_preclip = actor_norm_sq ** 0.5
                writer.add_scalar("update/actor_grad_norm_preclip", actor_grad_norm_preclip, global_step)
                writer.add_scalar(
                    "update/actor_grad_norm_postclip",
                    min(actor_grad_norm_preclip, args.max_grad_norm),
                    global_step,
                )
                ...
                for p in critic_params:
                    if p.grad is None:
                        continue
                    param_norm = p.grad.data.norm(2)
                    critic_norm_sq += param_norm.item() ** 2
                critic_grad_norm_preclip = critic_norm_sq ** 0.5
                writer.add_scalar("update/critic_grad_norm_preclip", critic_grad_norm_preclip, global_step)
                writer.add_scalar(
                    "update/critic_grad_norm_postclip",
                    min(critic_grad_norm_preclip, args.max_grad_norm),
                    global_step,
                )
                ...
                # --- param delta norm ---
                with torch.no_grad():
                    actor_params_before = [p.detach().clone() for p in actor_params]
                    critic_params_before = [p.detach().clone() for p in critic_params]
                actor_optimizer.step()
                critic_optimizer.step()
                with torch.no_grad():
                    actor_delta_sq = 0.0
                    for p0, p1 in zip(actor_params_before, actor_params):
                        d = (p1.detach() - p0)
                        actor_delta_sq += d.pow(2).sum().item()
                    writer.add_scalar("update/actor_delta_param_l2", actor_delta_sq ** 0.5, global_step)
                    critic_delta_sq = 0.0
                    for p0, p1 in zip(critic_params_before, critic_params):
                        d = (p1.detach() - p0)
                        critic_delta_sq += d.pow(2).sum().item()
                    writer.add_scalar("update/critic_delta_param_l2", critic_delta_sq ** 0.5, global_step)

                if args.target_kl is not None and approx_kl > args.target_kl:
                    stop_update = True
                    break
                
                # after critic_optimizer.step()
                p0 = next(iter(critic_optimizer.param_groups[0]["params"]))
                st = critic_optimizer.state[p0]
                if "last_update_norm" in st:
                    writer.add_scalar("update/critic_update_norm_postprecond", st["last_update_norm"].item(), global_step)
                    writer.add_scalar("update/critic_update_rms_postprecond", st["last_update_rms"].item(), global_step)
        ...
        writer.add_scalar("charts/learning_rate_actor", actor_optimizer.param_groups[0]["lr"], global_step)
        writer.add_scalar("charts/learning_rate_critic", critic_optimizer.param_groups[0]["lr"], global_step)
