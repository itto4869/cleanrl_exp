...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run PPO Atari SOAP rollout EMA mix from a config JSON."
    )
    parser.add_argument(
        "--config",
        default="experiments/mix_grad_clip_config.json",
        help="Path to mix_grad_clip_config.json with params and envs.",
    )
    parser.add_argument(
        "--total-timesteps",
        type=int,
...
def main():
    ...
    if not config["envs"]:
        raise ValueError("mix_grad_clip_config.json envs is empty")

    script = "experiments/ppo_atari_soap_rollout_ema_mix.py"
    ...
    param_args += [
        ...
        "--soap-update-clip-norm",
        "1.0",
        "--soap-trace-normalize-mode",
        "mean",
    ]

    seeds = parse_seeds(args.seeds)
    run_env = os.environ.copy()
    base_run_group = run_env.setdefault("RUN_GROUP", "mix_grad_clip_config_sweep")
    run_label = "soap-rollout-ema-mix"
